...
import '../../../custom_widgets/document_uploading.dart';
    ...
    return GetBuilder<KycController>(builder: (kycController) {
      List<Widget> children() {
        ...
        List<FormFieldItem> fields =
            kycController.updateProfileFormFZ!.formFieldItem.toList();
              ...
              l.add(
                Visibility(
                  ...
                  child: textFormField(
                      ...
                      onEditingComplete: () {
                        String? id = kycController
                            .updateProfileFormFZ?.formFieldItem
                            .toList()
                            .firstWhereOrNull((f0) =>
                                f0.index == f.index + 1 &&
            ...
            case FieldType.date:
              l.add(Visibility(
                ...
                child: dateField(
                    ...
                    ctrls:
                        kycController.form.dateFieldControllers[f.FzParamName]!,
                    ...
                    topMargin: 0,
                    initialDate: {
                          'DateOfBirth': DateTime(DateTime.now().year - allowedMinAge, DateTime.now().month, DateTime.now().day),
                        ...
                        DateTime.now(),
                    firstDate: {
                          'DateOfBirth': DateTime(DateTime.now().year - 100, DateTime.now().month, DateTime.now().day),
                          'IdIssueDate': DateTime(DateTime.now().year - 100, DateTime.now().month, DateTime.now().day),
                          'IdExpiryDate': DateTime.now(),
                    ...
                    lastDate: {
                          'DateOfBirth': DateTime(DateTime.now().year - allowedMinAge, DateTime.now().month, DateTime.now().day),
                          'IdIssueDate': DateTime.now(),
                          'IdExpiryDate': DateTime(DateTime.now().year + 30, DateTime.now().month, DateTime.now().day),
                          'dummy_salary_date': DateTime(DateTime.now().year + 30, DateTime.now().month, DateTime.now().day),
                        }[f.FzParamName] ??
                        DateTime.now(),
                    erfc: (b, {DateTime? date}) {
                      ...
                      if (date == null) {
                        ...
                        kycController.form.dates[f.FzParamName] =
                ...
                children: [
                  ...
                  Wrap(
                    children: (f.setOptions() ??
                            kycController.form.sources[f.ApiKey] ??
                            [])
                        .toList()
                        .map(
                          (e) => Padding(
      ...
      return GlobalScaffold(
        appBar: appBar0(cms(Fields.UpdateProfile_PageHeading).textHeading!,
            onTap2: () => Get.back()),
        body: SafeArea(
              ...
              : ListView(
                  children: [
                    Padding(
                      ...
                      child: Text(
                          Box3.settings.primaryDocIDName.isEmpty == true
                              ? 'National_ID'.tr
                              : "${Box3.settings.primaryDocIDName} ",
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(
                                  'front_side'.tr,
                                  style: TS.f12.copyWith(color: Colors.grey),
                                ),
                                SizedBox(
                                  height: 5,
                                ),
                            ...
                            child: kycController.loading
                                ...
                                : roundButton(
                                    ...
                                    if (kycController.validateEditProfile()) {
                                      ...
                                      List<bool> list = await Future.wait([
                                        ApiService().uploadMediaDocumentNew(
                                        ...
                                        //ApiService().updateCustomerProfile(DateTime(int.parse(dateArray[2]), int.parse(dateArray[1]), int.parse(dateArray[0])))
                                      ]);
