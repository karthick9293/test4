  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<AccountController>(builder: (accountController) {
      return GlobalScaffold(
        appBar: fromPrelogin
            ? appBar0('Track Remittance'.tr, onTap2: Get.back)
            : null,
        body: Column(
          children: [
            Container(
                ...
                children: [
                  Expanded(
                    child: textFormField(
                      ...
                      trailing: GestureDetector(
                        onTap: () {
                          if (accountController
                                  .refSearchController.text.isNotEmpty ==
                              true) {
                            accountController.trackRemittanceByRef(
                                accountController.refSearchController.text);
                          } else {
                      ...
                      errorText:
                          accountController.refErrorText.isNotEmpty == true
                              ? accountController.refErrorText
                              : '',
                    ),
                  ),
                ],
            ...
            Expanded(
              child: accountController.trackLoader == true
                  ? Center(child: loader())
                  : accountController.trackRemResult != null &&
                          !accountController.trackRemResult!.isSuccess
                      ? Center(
                          child: Text(
                            accountController.trackRemResult!.error
                                    ?.MessageDescription ??
                                'No data Found.',
                            style:
                                TS.f18.copyWith(color: AppConfigs.accentColor),
                          ),
                        )
                      : Visibility(
                          visible:
                              accountController.trackRemittanceData != null,
                          child: Container(
                            ...
                            child: ListView(
                              ...
                              children: [
                                ...
                                contentTile(
                                    'Payment Status'.tr,
                                    accountController.trackRemittanceData
                                            ?.transactionStatus ??
                                        '',
                                    isIcon: accountController
                                            .trackRemittanceData
                                            ?.transactionStatus
                                            ?.toLowerCase()
                                            .contains('success') ==
                                ...
                                contentTile(
                                  ...
                                  DateFormatter.newFormat(
                                      accountController
                                          .trackRemittanceData?.transactionDate
                                          ?.toLocal(),
                                      'dd/MM/yyyy hh:mm a'),
                                ),
