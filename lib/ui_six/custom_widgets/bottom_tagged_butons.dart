...
import '../screens/form_rules_form/add_beneficiary_form_rules.dart';
import 'error_alert.dart';
import 'mpin_alert.dart';
import 'otp_alert.dart';

class BottomButtons {
  static Widget addBeneficiary = GetBuilder<AddBeneficiaryController>(builder: (addBeneficiaryController) {
    return Visibility(
      ...
      child: roundButton(
        ...
        onTap: () async {
          ...
          Get
            ...
            ..find<MainController>().push(const AddBeneficiaryFormRules(), AppBars.addBeneficiary);
        },
      ),
    );
  });

  static Widget addBeneficiarySubmit = GetBuilder<AddBeneficiaryController>(builder: (addBeneficiaryController) {
    return addBeneficiaryController.addBeneficiaryButtonLoading
        ? loader()
        : roundButton(cms(Fields.PopUp_Submit).textHeading!, left: 20, top: 20, right: 20, bottom: 20, radius: 10, onTap: () async {
            if (AppConfigs.formRules) {
              if (!addBeneficiaryController.form
                  .validateNew(addBeneficiaryController.beneficiaryFormBV!.formFieldItem, validate: addBeneficiaryController.visibilityChecker)) {
                ...
                if (addBeneficiaryController.benefAttributes.isAddBeneficiary) {
                  switch (Box3.settings.authBenfAdd) {
                    case AuthMode.mPin:
                      Get.find<AuthController>().dialogAuthAttributes = DialogMpinAuthAttributes();
                      await validateMPIN(onCompleted: addBeneficiaryController.addBeneficiary);
                      addBeneficiaryController
                    ...
                    case AuthMode.otpPhone:
                      addBeneficiaryController
                        ..addBeneficiaryButtonLoading = true
                        ..update();
                      RegisterDeviceFZ? reg = await ApiService().createOTP(OtpType.CREATE_ADDBENEFICIARY_OTP);
                      if (reg != null && reg.data != null) {
                        ...
                        validateOTP(
                            ...
                            onValidate: () async {
                              if (t.dialogOtpAuthAttributes.mobOTP.text.length < 6) {
                                t
                                ...
                                Map<String, dynamic> validateOtpRes = await ApiService().validateOtpRequest(
                                    otpID: reg!.data!.notificationArray!.first.otpId!, otp: t.dialogOtpAuthAttributes.mobOTP.text);
                                CustomMessageDetailsFZ? customMessageDetails = validateOtpRes['message'];
                                ...
                                if (isVerified) {
                                  ...
                                  t
                                    ..errorText = customMessageDetails?.MessageDescription ?? 'Invalid OTP'
                                    ..buttonLoading = false
                                    ..dialogOtpAuthAttributes.mobOTP.clear()
                                    ..update();
                                }
                  ...
                  switch (Box3.settings.authBenfEdit) {
                    case AuthMode.mPin:
                      Get.find<AuthController>().dialogAuthAttributes = DialogMpinAuthAttributes();
                      await validateMPIN(onCompleted: addBeneficiaryController.updateBeneficiary);
                      addBeneficiaryController
                        ..addBeneficiaryButtonLoading = true
                        ..loading = false
                        ..update();
  ...
  static Widget download = GetBuilder<TransactionController>(builder: (transactionController) {
    return Obx(
      () => Visibility(
        visible: transactionController.transactions
            .where((t) =>
                (transactionController.name.isNotEmpty
                    ? (t.beneficiaryName ?? '').toLowerCase().contains(transactionController.name.toLowerCase()) ||
                        (t.transactionReference ?? '').toLowerCase().contains(transactionController.name.toLowerCase())
                    : true) &&
                (transactionController.status.isNotEmpty && t.transactionStatus != null
                    ? (transactionController.status.contains('Processing') && t.transactionStatus!.toLowerCase().contains('processing')) ||
                        (transactionController.status.contains('Success') && t.transactionStatus!.toLowerCase().contains('success')) ||
                        (transactionController.status.contains('Failed') && t.transactionStatus!.toLowerCase().contains('failed')) ||
                        (transactionController.status.contains('Cancelled') && t.transactionStatus!.toLowerCase().contains('cancelled'))
                    : true) &&
                (transactionController.paymentInstrument.isNotEmpty
                    ...
                    : true) &&
                (transactionController.startDate != null && t.transactionDate != null
                    ? t.transactionDate!.toLocal().isAfter(transactionController.startDate!)
                    : true) &&
                (transactionController.endDate != null && t.transactionDate != null
                    ? t.transactionDate!.toLocal().isBefore(transactionController.endDate!)
                    : true))
            .toList()
            .isNotEmpty,
        child: transactionController.downloadingStatement.isTrue
            ? loader(bottom: 20)
            : roundButton(cms(Fields.MyTransaction_DownloadButton).textHeading!, bottom: 20, top: 20, onTap: () {
    ...
    builder: (transferController) {
      return (transferController.buttonLoading)
          ...
          : roundButton(
              ...
              top: 20,
              onTap: () async {
                double fAmt = double.tryParse(transferController.fromAmountCtrl.text) ?? 0;
                double tAmt = double.tryParse(transferController.toAmountCtrl.text) ?? 0;
                if (fAmt == 0) {
                  transferController
                  ...
                  transferController
                    ...
                    ..update();
                } else if (transferController.selectedBeneficiary == null && Get.context != null) {
                ...
                } else if (transferController.selectedPurpose == null || transferController.selectedSource == null) {
                  transferController.sourceError = transferController.selectedSource == null;
                  transferController.purposeError = transferController.selectedPurpose == null;
                  transferController.update();
                  GlobalKey? targetKey;
                  ...
                  if (transferController.rC?.statusCode == 'CC-SUCC') {
                    Get.find<MainController>().push(AppRoutes.getPage(AppRoutes.confirmPaymentDetails), AppBars.confirmPaymentDetails);
                  } else if (transferController.rC?.statusCode == '403') {
                    errorAlert(content: transferController.rC!.customMessageDetails?.messageDescription ?? "");
                  }
                }
              },
            );
  ...
  static Widget sendMoneyNext2 = GetBuilder<TransferController>(builder: (transferController) {
    return roundButton(cms(Fields.SendMoney_ProceedButton).textHeading!, top: 20, onTap: () {
      transferController
        ..purposeError = transferController.selectedPurpose == null
        ..sourceError = transferController.selectedSource == null
        ..update();
      if (((transferController.selectedBeneficiary?.serviceType?.Code.toUpperCase() == 'C' ||
        ...
        Get.find<MainController>().push(PaymentModeScreen(), AppBars.confirmPaymentDetails);
      }
    });
  });
}
