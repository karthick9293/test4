...
void sourceBottomSheet(
  ...
  Widget titleTile(String title, bool isDark) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Text(title,
              style: TextStyles.font14.copyWith(color: isDark ? AppConfigs.w : const Color(0xff2d2d2d), fontWeight: FontWeight.w700),
  ...
  Widget contentTile({required MasterResponseFZ sourceBV, bool showCode = true, required bool isDark}) {
    return Container(
      ...
      child: GestureDetector(
        onTap: () {
          onTap(sourceBV);
          if (selected != null) {
            List<MasterResponseFZ> temp = filteredSources.where((s) => selected.contains(s)).toList();
            ...
            Expanded(
              child: Container(
                ...
                child: Text(sourceBV.Desc,
                    style: TextStyles.font14.copyWith(color: isDark ? AppConfigs.g8f : AppConfigs.g4, fontWeight: FontWeight.w600),
                    textAlign: TextAlign.start),
              ),
            ),
            Visibility(
      ...
      child: Column(
        children: [
          ...
          textFormField(
              ...
              onChanged: (s) {
                if (s.isNotEmpty) {
                  filteredSources = sources.where((c) => c.Desc.toLowerCase().contains(s.toLowerCase()) || selected?.contains(c) == true).toList();
          ...
          Expanded(
              child: ListView.builder(
                  ...
                  itemBuilder: (c, i) => contentTile(sourceBV: filteredSources[i], isDark: signUpController.isDark)))
        ],
      ),
    );
  }));
