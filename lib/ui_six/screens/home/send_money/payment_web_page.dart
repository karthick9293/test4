...
class _PaymentWebPageState extends State<PaymentWebPage> {
  ...
  Future<void> safeClearSslPreferences(InAppWebViewController controller) async {
    if (kIsWeb) return;
              ...
              InAppWebView(
                ...
                onUpdateVisitedHistory: (controller, url, androidIsReload) async {
                  ...
                  if (uri != null) {
                    ...
                    if (uri.path.contains('/fz-api/') && uri.queryParameters.containsKey('id')) {
  ...
  Future<void> handlePaymentRedirect(Uri uri) async {
    ...
    try {
      ...
      if (id == 1) {
        if (AppConfigs.fetchNewConfig ? Box3.settings.amlForTransaction : Box3.settings.amlForTransaction) {
          if (await ApiService().validateMandatory(BankAttributes.referenceNumber)) {
            TransactionDetailsFz? td = await ApiService().transactionDetailsByRefNo(
        ...
        String? message = uri.queryParameters['sStatusMessage']?.replaceAll('%20', ' ');
