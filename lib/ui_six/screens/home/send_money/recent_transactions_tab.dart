      ...
      children: [
        Row(
          children: [
            Expanded(
              child: textFormField(
                hintText: cms(Fields.MyBeneficiary_SearchBeneficiary).textHeading!,
                ...
                onChanged: (v) {
                  if (!transferController.listLoading) {
                    transferController
                      ..filteredRecentTransactions = transferController.recentTransactions
                          .where((e) => (v.isNotEmpty ? e.beneficiaryName!.toLowerCase().contains(v.toLowerCase()) : true))
                          .toList()
                      ..update();
                  }
                },
                prefixIcon: Padding(
                  ...
                  child: SvgPicture.asset(
                    ...
                    height: 11.8,
                    width: 12,
                    colorFilter: ColorFilter.mode(Color(0xffd1d1d1), BlendMode.srcIn),
                  ),
                ),
              ),
            ),
        ...
        AnimatedContainer(
          ...
          decoration: BoxDecoration(borderRadius: const BorderRadius.all(Radius.circular(19)), border: Border.all(color: AppConfigs.gd7)),
          child: transferController.listLoading || transferController.syncingBeneficiaries
              ? BeneficiaryShimmer(
                  profileHeight: 50,
                  itemLength: 5,
                )
              : transferController.filteredRecentTransactions.isEmpty
                  ...
                  : Scrollbar(
                      ...
                      child: ListView.separated(
                        ...
                        itemCount:
                            transferController.filteredRecentTransactions.length > 5 ? 5 : transferController.filteredRecentTransactions.length,
                        ...
                        itemBuilder: (c, i) => transactionTile(
                          selected: transferController.filteredRecentTransactions[i].transactionReference ==
                              transferController.selectedTransaction?.transactionReference,
                          transaction: transferController.filteredRecentTransactions[i],
                          hide: true,
                          onTap: () {
                            transferController.setBenefFromTxn(transferController.filteredRecentTransactions[i]);
                          },
                        ),
                      ),
                    ),
