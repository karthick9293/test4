  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<SignUpController>(builder: (signUpController) {
      List<Widget> children() {
        List<Widget> l = [];
        List<FormFieldItem> fields = signUpController.credFormBV!.formFieldItem.toList();
        ...
        for (FormFieldItem f in fields) {
          switch (f.FieldType) {
            case FieldType.username:
              l.add(
                textFormField(
                    controller: signUpController.credForm.textFieldControllers[f.FzParamName],
                    ...
                    inputFormatters: f.Regex.isNotEmpty
                        ...
                        : null,
                    errorText: signUpController.credForm.errorTexts[f.FzParamName]!,
                    enabled: f.enabled,
                    onChanged: (v) async {
                      if (signUpController.credForm.errorTexts[f.FzParamName]!.isNotEmpty) {
                      ...
                      if (v.length > 3) {
                        ...
                        if (signUpController.signUpAttributes.usernameAvailable) {
                          signUpController.credForm.errorTexts[f.FzParamName] = '';
                          signUpController.update();
                        } else {
                          signUpController.credForm.errorTexts[f.FzParamName] = signUpController.signUpAttributes.usernameError;
                          signUpController.update();
                        }
                      }
                    },
                    ...
                    trailing: signUpController.signUpAttributes.usernameLoading
                        ? SizedBox(width: 25, height: 25, child: Center(child: tinyLoader(size: 20, width: 2)))
                        : null,
                    prefixText: f.Prefix),
              );
              break;
            case FieldType.number:
              l.add(
                textFormField(
                    ...
                    enabled: f.enabled,
                    obscure: signUpController.credForm.visibilities[f.FzParamName] == true,
                    obscuringCharacter: f.Masking != "" ? f.Masking : null,
                    trailing: f.Masking != ""
                        ? GestureDetector(
                            onTap: () async {
                              if (signUpController.credForm.visibilities[f.FzParamName]!) {
                                signUpController
                                  ..credForm.visibilities[f.FzParamName] = false
                                  ..update();
                                await Future.delayed(const Duration(seconds: 2));
                                signUpController
                                  ..credForm.visibilities[f.FzParamName] = true
                                  ..update();
                              }
                            },
                            child: Icon(
                              signUpController.credForm.visibilities[f.FzParamName] == true ? Icons.visibility_off : Icons.visibility,
                              size: 20,
            ...
            case FieldType.text:
              ...
              if (f.FzParamName == "Password" || f.FzParamName == 'CONFIRMPASSWORD') {
                fieldVisibility =
                    Box3.settings.LOGINAUTHENTICATIONSECONDARY == "PASSWORD" || Box3.settings.LOGINAUTHENTICATIONSECONDARY == "PASSWORD_MPIN";
              }
              l.add(
                Visibility(
                  ...
                  child: textFormField(
                      ...
                      onChanged: (v) {
                        ...
                        if (f.FieldId == 'newPassword' || f.FieldId == 'confirmPassword') {
                          signUpController
                    ...
                    Padding(
                      ...
                      child: Text(
                          signUpController.signUpAttributes.pwdStrength < 0.34
                              ? 'weak'.tr
                              : signUpController.signUpAttributes.pwdStrength < 0.68
                                  ? 'Medium'.tr
                                  : 'Strong'.tr,
                          style: TS.f14),
                    ),
                      ...
                      child: Column(
                        children: [
                          textConstratint('contains_at_least_one_uppercase'.tr,
                              signUpController.credForm.textFieldControllers[f.FzParamName]!.text.contains(RegExp(r'[A-Z]'))),
                          textConstratint('contains_at_least_one_lowercase'.tr,
                              signUpController.credForm.textFieldControllers[f.FzParamName]!.text.contains(RegExp(r'[a-z]'))),
                          textConstratint('contains_at_least_one_number'.tr,
                              signUpController.credForm.textFieldControllers[f.FzParamName]!.text.contains(RegExp(r'[0-9]'))),
                          textConstratint('contains_at_least_one_special_char'.tr,
                              signUpController.credForm.textFieldControllers[f.FzParamName]!.text.contains(RegExp(r'[!@#\$&*~]'))),
                          textConstratint('Password length 8-$pwdMaxLength characters',
                              signUpController.credForm.textFieldControllers[f.FzParamName]!.text.length > 7)
                        ],
                      ),
                    ),
                  ],
            ...
            case FieldType.date:
              l.add(dummyTextField(
                ...
                if (d != null) {
                  signUpController
                    ..form.dates[f.FzParamName] = DateFormatter.newFormat(d, 'dd/MMM/yyyy')
            ...
            case FieldType.dropDownSingle:
              l.add(genericBottomSheet(
                  ...
                  labelText: '${f.FieldDisplayName}${f.optional ? '' : '*'}',
                  hintText: f.FieldDisplayName,
                  value: signUpController.form.selectedSources[f.FzParamName]?.Desc ?? '',
                  credKey: f.ApiKey,
            ...
            case FieldType.radio:
              l.add(Column(
                ...
                children: [
                  ...
                  Wrap(
                    children: (f.setOptions() ?? signUpController.form.sources[f.ApiKey]!)
                        .toList()
                        .map(
                          (e) => Padding(
                            padding: const EdgeInsets.only(left: 20, top: 5),
                            child: radioTile(signUpController.form.selectedSources[f.FzParamName]?.id == e.id, e.Desc.tr,
      ...
      return Scaffold(
        appBar: appBar0(cms(Fields.SetCredential_PageHeading).textHeading!, onTap2: Get.back),
                        ...
                        : roundButton(
                            ...
                            onTap: () async {
                              if (Box3.settings.processEkycAs == 'UAEPASS') {
                                ...
                                if (!signUpController.credForm
                                    .validateNew(signUpController.credFormBV!.formFieldItem, validate: signUpController.visibilityChecker)) {
                                  signUpController.update();
                                } else {
                                  Map? result;
                                  signUpController
                                  ...
                                  if (setCredential) {
                                    ...
                                    result = await signUpController.validateNid(
                                        ...
                                        authData: signUpController.credForm.buildJsonNew(signUpController.credFormBV!.formFieldItem)
                                          ..addAll({
                                            "UUID": Get.find<SignUpController>().uaePassProfileFz?.uuid ?? '',
                                            "EMAIL": Get.find<SignUpController>().ivAttributes.updateEmail ?? '',
                                          }),
                                        stepOrderId: stepOrderId);
                                  } else {}
                                  signUpController
