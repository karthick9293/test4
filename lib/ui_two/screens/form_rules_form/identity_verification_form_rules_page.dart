...
class IdentityVerificationFormRulesPage extends StatefulWidget {
  ...
  @override
  State<IdentityVerificationFormRulesPage> createState() => _IdentityVerificationFormRulesPageState();
}

class _IdentityVerificationFormRulesPageState extends State<IdentityVerificationFormRulesPage> {
  ...
  void initializeControllers() {
    for (var field in controller.idVerificationFormBV!.formFieldItem) {
      controller.idVerificationForm.textFieldControllers[field.FzParamName] = TextEditingController(text: '');
      controller.idVerificationForm.dateFieldControllers[field.FzParamName] = [TextEditingController(text: ''),TextEditingController(text: ''),TextEditingController(text: '')];
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<SignUpController>(builder: (signUpController) {
      List<Widget> children() {
        List<Widget> l = [];
        List<FormFieldItem> fields = signUpController.idVerificationFormBV!.formFieldItem.toList();
          ...
          switch (f.FieldType) {
            case FieldType.number:
              ...
              if (f.FzParamName == "IdNo") {
                trailingIcon = Get.find<AccountController>().isDark ? ImagePath.darkIdentification : ImagePath.identification;

                if (signUpController.customerAuthData['IdNo'] != null) {
                  signUpController.idVerificationForm.textFieldControllers[f.FzParamName]?.text = signUpController.customerAuthData['IdNo'];
                  isEnabled = false;
                }
              }
              ...
              l.add(
                Visibility(
                  ...
                  child: textFormField(
                      ...
                      controller: signUpController.idVerificationForm.textFieldControllers[f.FzParamName],
                      labelText: f.FieldDisplayName + (f.optional ? '' : ' *'),
                      // inputFormatters: [FilteringTextInputFormatter.allow(RegExp(f.Regex))],
                      focusNode: signUpController.idVerificationForm.nodes[f.FzParamName],
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                      errorText: signUpController.idVerificationForm.errorTexts[f.FzParamName]!,
                      maxLength: f.maxLength,
                      enabled: isEnabled,
                      onChanged: (v) {
                        if (signUpController.idVerificationForm.errorTexts[f.FzParamName]!.isNotEmpty) {
                          signUpController.idVerificationForm.errorTexts[f.FzParamName] = '';
                      ...
                      onEditingComplete: () {
                        String? id = signUpController.idVerificationFormBV?.formFieldItem
                            .toList()
                      ...
                      obscure: signUpController.idVerificationForm.visibilities[f.FzParamName] == true,
                      obscuringCharacter: f.Masking != "" ? f.Masking : null,
                      trailing: f.Masking != ""
                          ? GestureDetector(
                              onTap: () async {
                                if (signUpController.idVerificationForm.visibilities[f.FzParamName]!) {
                                  signUpController
                                    ..idVerificationForm.visibilities[f.FzParamName] = false
                                    ..update();
                                  await Future.delayed(const Duration(seconds: 2));
                                  signUpController
                                    ..idVerificationForm.visibilities[f.FzParamName] = true
                                    ..update();
                                }
                              },
                              child: Icon(
                                signUpController.idVerificationForm.visibilities[f.FzParamName] == true ? Icons.visibility_off : Icons.visibility,
                                size: 20,
            ...
            case FieldType.date:
              l.add(dateField(
                key: signUpController.idVerificationForm.keys[f.Id.toString()],
                context: context,
                ctrls: signUpController.idVerificationForm.dateFieldControllers[f.FzParamName]!,
                nodes: signUpController.idVerificationForm.dateFieldNodes[f.FzParamName]!,
                error: signUpController.idVerificationForm.errorTexts[f.FzParamName]!,
                update: signUpController.update,
                title: '${f.FieldDisplayName}${f.optional ? '' : ' *'}',
                erfc: (b, {DateTime? date}) {
                  //TODO: Handle age limit here
                  if (date == null) {
                    signUpController.idVerificationForm.dates.remove(f.FzParamName);
                  } else {
                    signUpController.idVerificationForm.dates[f.FzParamName] = DateFormatter.newFormat(date, 'dd/MMM/yyyy');
                  }

                  signUpController.idVerificationForm.errorTexts[f.FzParamName] = b ? '' : 'Invalid_date'.tr;
                  signUpController.update();
                },
                onComplete: () {
                  String? id = signUpController.idVerificationFormBV?.formFieldItem
                ...
                lastDate: {
                      'DOB': DateTime(
                          ...
                          DateTime.now().day),
                      'dummy_idissueDate': DateTime.now(),
                      'IDEXPIRY': DateTime.now().add(const Duration(days: 365 * 30)),
                      'dummy_salary_date': DateTime(DateTime.now().year + 30, DateTime.now().month, DateTime.now().day),
                    }[f.FzParamName] ??
                    DateTime.now(),
              ));
            ...
            case FieldType.dropDownSingle:
              l.add(genericBottomSheet(
                  ...
                  labelText: '${f.FieldDisplayName}${f.optional ? '' : ' *'}',
                  value: signUpController.idVerificationForm.selectedSources[f.FzParamName]?.Desc ?? '',
                  f: f,
                  credKey: f.ApiKey,
                  errorText: signUpController.idVerificationForm.errorTexts[f.FzParamName]!,
                  onTap: (s) {
            ...
            case FieldType.radio:
              l.add(Column(
                ...
                children: [
                  ...
                  Wrap(
                    children: (f.setOptions() ?? signUpController.idVerificationForm.sources[f.ApiKey]!)
                        .toList()
                        .map(
                          (e) => Padding(
                            padding: const EdgeInsets.only(left: 20, top: 5),
                            child: radioTile(signUpController.idVerificationForm.selectedSources[f.FzParamName]?.id == e.id, e.Desc.tr,
                                onTap: () => signUpController
                                  ..idVerificationForm.selectedSources[f.FzParamName] = e
                                  ..idVerificationForm.errorTexts[f.FzParamName] = ''
                                  ..update()),
                          ),
                        )
                  ...
                  children: [
                      ...
                      children: signUpController.idVerificationFormBV == null
                          ...
                          : [
                              ...
                              FormLoggerView(
                                ...
                                formRuleData: signUpController.idVerificationFormBV,
                              ),
                            ],
                    ),
                    ...
                    signUpController.idVerificationFormBV != null
                        ? signUpController.loading
                            ...
                            : roundButton(cms(Fields.IdentityVerification_VerifyButton).textHeading!, bottom: 10, onTap: () async {
                                {
                                  bool status = signUpController.idVerificationForm.validateNew(signUpController.idVerificationFormBV!.formFieldItem);
                                  debugPrint("identity verification");
                                  if (status) {
                                    Map<String, dynamic> authData =
                                        signUpController.idVerificationForm.buildJsonNew(signUpController.idVerificationFormBV!.formFieldItem);
