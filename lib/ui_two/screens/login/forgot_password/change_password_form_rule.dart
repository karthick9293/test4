...
class _ChangePasswordFormRuleState extends State<ChangePasswordFormRule> {
  ...
  void initializeControllers() {
    for (var field in a.changePasswordFormFZ!.formFieldItem) {
      a.changePasswordForm.textFieldControllers[field.FzParamName] = TextEditingController(text: '');
  ...
  Widget build(BuildContext context) {
    return GetBuilder<AccountController>(builder: (accountController) {
      List<Widget> children() {
        List<Widget> l = [];
        List<FormFieldItem> fields = accountController.changePasswordFormFZ!.formFieldItem.toList()..sort((a, b) => a.index.compareTo(b.index));
        for (FormFieldItem f in fields) {
          switch (f.FieldType) {
            case FieldType.number:
              ...
              l.add(
                Visibility(
                  ...
                  child: textFormField(
                      controller: accountController.changePasswordForm.textFieldControllers[f.FzParamName],
                      ...
                      inputFormatters: f.Regex.isNotEmpty
                          ...
                          : null,
                      focusNode: accountController.changePasswordForm.nodes[f.FzParamName],
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                      errorText: accountController.changePasswordForm.errorTexts[f.FzParamName]!,
                      maxLength: f.maxLength,
                      enabled: f.enabled,
                      onChanged: (v) {
                        if (accountController.changePasswordForm.errorTexts[f.FzParamName]!.isNotEmpty) {
                          accountController.changePasswordForm.errorTexts[f.FzParamName] = '';
                      ...
                      onEditingComplete: () {
                        String? id = accountController.changePasswordFormFZ?.formFieldItem
                            .toList()
                            .firstWhereOrNull((f0) => f0.index == f.index + 1 && (f0.FieldType == FieldType.text || f0.FieldType == FieldType.number))
                            ?.FzParamName;
                        if (id != null) {
                          accountController.changePasswordForm.nodes[id]?.requestFocus();
                        } else {
                          Get.focusScope?.unfocus();
                        }
                      },
                      obscure: accountController.changePasswordForm.visibilities[f.FzParamName] == true,
                      obscuringCharacter: f.Masking != "" ? f.Masking : null,
                      trailing: f.Masking != ""
                          ? GestureDetector(
                              onTap: () async {
                                if (accountController.changePasswordForm.visibilities[f.FzParamName]!) {
                                  accountController
                                    ..changePasswordForm.visibilities[f.FzParamName] = false
                                    ..update();
                                  await Future.delayed(const Duration(seconds: 2));
                                  accountController
                                    ..changePasswordForm.visibilities[f.FzParamName] = true
                                    ..update();
                                }
                              },
                              child: Icon(
                                accountController.changePasswordForm.visibilities[f.FzParamName] == true ? Icons.visibility_off : Icons.visibility,
                                size: 20,
              ...
              l.add(
                textFormField(
                    ...
                    onChanged: (v) {
                      ...
                      if (f.FieldId == 'password' || f.FieldId == 'confirmPassword') {
                        accountController
                          ..signUpAttributes.pwdStrength = (v.contains(RegExp(r'[A-Z]')) ? 0.20 : 0) +
                    ...
                    Padding(
                      ...
                      child: Text(
                          accountController.signUpAttributes.pwdStrength < 0.34
                              ? 'weak'.tr
                              : accountController.signUpAttributes.pwdStrength < 0.68
                                  ? 'Medium'.tr
                                  : 'Strong'.tr,
                          style: TS.f14),
                    ),
                      ...
                      child: Column(
                        children: [
                          textConstratint('contains_at_least_one_uppercase'.tr,
                              accountController.changePasswordForm.textFieldControllers[f.FzParamName]!.text.contains(RegExp(r'[A-Z]'))),
                          textConstratint('contains_at_least_one_lowercase'.tr,
                              accountController.changePasswordForm.textFieldControllers[f.FzParamName]!.text.contains(RegExp(r'[a-z]'))),
                          textConstratint('contains_at_least_one_number'.tr,
                              accountController.changePasswordForm.textFieldControllers[f.FzParamName]!.text.contains(RegExp(r'[0-9]'))),
                          textConstratint('contains_at_least_one_special_char'.tr,
                              accountController.changePasswordForm.textFieldControllers[f.FzParamName]!.text.contains(RegExp(r'[!@#\$&*~]'))),
                          textConstratint('Password length 8-$pwdMaxLength characters',
                              accountController.changePasswordForm.textFieldControllers[f.FzParamName]!.text.length > 7)
                        ],
                      ),
                    ),
                  ],
            ...
            case FieldType.date:
              l.add(dummyTextField(accountController.changePasswordForm.dates[f.FzParamName], accountController.isDark,
                  error: accountController.changePasswordForm.errorTexts[f.FzParamName]!,
                ...
                if (d != null) {
                  accountController
                    ..changePasswordForm.dates[f.FzParamName] = DateFormatter.newFormat(d, 'dd/MMM/yyyy')
            ...
            case FieldType.dropDownSingle:
              l.add(genericBottomSheet(
                  ...
                  labelText: '${f.FieldDisplayName}${f.optional ? '' : ' *'}',
                  value: accountController.changePasswordForm.selectedSources[f.FzParamName]?.Desc ?? '',
                  credKey: f.ApiKey,
                  f: f,
                  errorText: accountController.changePasswordForm.errorTexts[f.FzParamName]!,
                  onTap: (s) {
            ...
            case FieldType.radio:
              l.add(Column(
                ...
                children: [
                  ...
                  Wrap(
                    children: (f.setOptions() ?? accountController.changePasswordForm.sources[f.ApiKey]!)
                        .toList()
                        .map(
                          (e) => Padding(
                            padding: const EdgeInsets.only(left: 20, top: 5),
                            child: radioTile(accountController.changePasswordForm.selectedSources[f.FzParamName]?.id == e.id, e.Desc.tr,
                                onTap: () => accountController
                                  ..changePasswordForm.selectedSources[f.FzParamName] = e
                                  ..changePasswordForm.errorTexts[f.FzParamName] = ''
                                  ..update()),
                          ),
                        )
      ...
      return Scaffold(
        ...
        appBar: appBar0(cms(Fields.ForgotPassword_PageHeading).textHeading!, onTap2: Get.back),
          ...
          child: ListView(
            ...
            children: [
              ...
              ...accountController.changePasswordFormFZ == null
                  ...
                  : [
                      ...children(),
                      FormLoggerView(formName: FormNames.changePasswordFormRules, formRuleData: accountController.changePasswordFormFZ),
                      accountController.buttonLoading
                          ? loader(top: 20, bottom: 5)
                          : roundButton(cms(Fields.ForgotMPIN_SendButton).textHeading!, top: 20, bottom: 5, onTap: () async {
                              bool status = accountController.changePasswordForm.validateNew(accountController.changePasswordFormFZ!.formFieldItem);
                              if (status) {
                                a
                                  ..buttonLoading = true
                                  ..update();
                                ...
                                try {
                                  var data = accountController.changePasswordForm.buildJsonNew(accountController.changePasswordFormFZ!.formFieldItem);
                                  ...
                                  await a.forgetMpin(stepOrderId: a.stepOrderID, data: data);
                                } catch (e) {
