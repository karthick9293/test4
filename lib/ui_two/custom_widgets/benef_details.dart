...
Padding benefDetails(
  ...
  Map<String, String> serviceTypeIcons = {
  ...
  List<Map<String, String>> contents = transferController
              .selectedBeneficiary?.serviceType?.Code
              .toUpperCase() ==
          'B'
      ? [
          {
            'key': cms(Fields.ConfirmPaymentDetail_AccountNo).textHeading!,
            'value': '${transferController.selectedBeneficiary!.accountNumber}'
          },
          {
            'key': cms(Fields.ConfirmPaymentDetail_BankName).textHeading!,
            'value': '${transferController.selectedBeneficiary!.bank?.Desc}'
          },
          if (transferController.selectedBeneficiary!.branch?.Desc.isNotEmpty ??
              false)
            {
              'key': cms(Fields.ConfirmPaymentDetail_BranchName).textHeading!,
              'value': '${transferController.selectedBeneficiary!.branch?.Desc}'
            },
        ]
      : transferController.selectedBeneficiary?.serviceType?.Code
          ...
          ? [
              {
                'key': cms(Fields.ConfirmPaymentDetail_Location).textHeading!,
                'value':
                    (transferController.selectedBeneficiary!.address1 ?? '--')
              },
              {
                'key': cms(Fields.ConfirmPaymentDetail_Country).textHeading!,
                'value':
                    transferController.selectedBeneficiary!.country?.Desc ?? '-'
              },
            ]
          : [
              {
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  transferController.selectedBeneficiary!.firstName ?? '',
                ...
                Container(
                  ...
                  child: Center(
                    child: SvgPicture.asset(
                      serviceTypeIcons[serviceType.Code.toUpperCase()]!,
                    ),
                  ),
                ),
              SizedBox(width: 5),
