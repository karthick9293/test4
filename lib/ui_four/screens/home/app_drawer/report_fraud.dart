...
class ReportFraudPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final bool isDark = Get.isRegistered<MainController>()
        ? Get.find<MainController>().isDark
        : false;
    return Scaffold(
      backgroundColor: (isDark ? AppConfigs.b : AppConfigs.w),
      appBar:
          isPrelogin ? appBar0('Report a Fraud'.tr, onTap2: Get.back) : null,
      body: Container(
        ...
        child: SafeArea(
          child:
            ...
            padding: const EdgeInsets.only(
                top: 8.0, right: 1.0, left: 8.0, bottom: 10),
            child: Scrollbar(
              ...
              child: SingleChildScrollView(
                ...
                child: Container(
                  ...
                  padding: const EdgeInsets.only(
                      top: 8.0, right: 1.0, left: 8.0, bottom: 8.0),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      title(
                          "${Box3.companyDataFZ?.companySettings?.companyName} – Fraud Awareness Guide"),
                      buildContactLink(
                          "📞 Customer Care: ${Box3.companyDataFZ?.companySettings?.phoneNumber}",
                          "tel:${Box3.companyDataFZ?.companySettings?.phoneNumber}"),
                      buildEmailLink(
                          "📧 Email: ${Box3.companyDataFZ?.companySettings?.emailId}",
                          "${Box3.companyDataFZ?.companySettings?.emailId}"),
                      buildContactLink(
                          "🌐 Website: https://${Box3.companyDataFZ?.companySettings?.companyName.toLowerCase().replaceAll(" ", "")}.ae",
                      ...
                      paragraph(
                          "If something feels suspicious—it probably is. Always verify the source before acting on any request or offer."),
                      sectionTitle("Report Fraud Immediately"),
                      paragraph(
                          "Your awareness protects not just you, but everyone. If you suspect any fraudulent activity:"),
                      buildContactLink("📞 Customer Care: +971 600 54 0002",
                          "tel:+971600540002"),
                      buildEmailLink(
                          "📧 Email: ${Box3.companyDataFZ?.companySettings?.emailId}",
                          "${Box3.companyDataFZ?.companySettings?.emailId}"),
                      sectionTitle(
                          "You can also report fraud to the authorities:"),
                      subtitle("Dubai Police"),
                      buildContactLink("• Toll-free: 901", "tel:901"),
                      buildContactLink(
                          "• E-Crime Platform: www.dubaipolice.gov.ae",
                          "https://www.dubaipolice.gov.ae"),
                      subtitle("Abu Dhabi Police"),
                      buildContactLink(
                          "• “Aman” Hotline: 800-2626", "tel:8002626"),
                      buildContactLink("• www.adpolice.gov.ae",
                          "https://www.adpolice.gov.ae"),
                      subtitle("Ministry of Interior"),
                      buildContactLink("• Hotline: 8005000", "tel:8005000"),
                      buildEmailLink(
                          "• Email: moi@moi.gov.ae", "moi@moi.gov.ae"),
                      ...
                      paragraph(
                          "✔ ${Box3.companyDataFZ?.companySettings?.companyName} will never:"),
                      bulletPoints([
                      ...
                      paragraph(
                          "Your security is our top priority. Together, let’s prevent fraud and build a safer financial community."),
                      sectionTitle(
                          "${Box3.companyDataFZ?.companySettings?.companyName}"),
                      buildContactLink(
                          "📞 ${Box3.companyDataFZ?.companySettings?.phoneNumber}",
                          "tel:${Box3.companyDataFZ?.companySettings?.phoneNumber}"),
                      buildEmailLink(
                          "📧 ${Box3.companyDataFZ?.companySettings?.emailId}",
                          "${Box3.companyDataFZ?.companySettings?.emailId}"),
                      buildContactLink(
                          "🌐 www.${Box3.companyDataFZ?.companySettings?.companyName.toLowerCase().replaceAll(" ", "")}.com",
