...
class _IDScanScreenState extends State<IDScanScreen>
  ...
  Future<void> _initializeCamera() async {
    try {
      ...
      setState(() {
        _isInitialized = true;
        _statusMessage = widget.captureType == 'front'
            ? 'Position ID FRONT side (with photo) in the frame'
            : 'Position ID BACK side in the frame';
      });
  ...
  Uint8List _yuv420ToNv21(CameraImage image) {
    ...
    final nv21 = Uint8List(
        yPlane.bytes.length + uPlane.bytes.length + vPlane.bytes.length);
  ...
  Uint8List _concatenatePlanes(List<Plane> planes) {
    final buffer =
        Uint8List(planes.fold(0, (sum, plane) => sum + plane.bytes.length));
    var offset = 0;
  ...
  bool _detectPhoto(List<Face> faces, String text) {
    ...
    final frontMatches =
        frontIndicators.where((pattern) => pattern.hasMatch(cleanText)).length;
  ...
  bool _isEmiratesId(String cleanText) {
    ...
    int emiratesMatches = emiratesIndicators
  ...
  bool _isValidEmiratesIdFront(String cleanText) {
    ...
    if (RegExp(r'(Date of Birth|DOB):?.*\d{1,2}[/-]\d{1,2}[/-]\d{4}')
        .hasMatch(cleanText)) {
      score += 3;
  ...
  bool _isValidEmiratesIdBack(String cleanText) {
    ...
    if (RegExp(r'(Issuing Place|Dubai|ABU DHABI|SHARJAH|AJMAN)',
            caseSensitive: false)
        .hasMatch(cleanText)) {
      score += 3;
    ...
    int arabicMatches =
        arabicPatterns.where((p) => p.hasMatch(cleanText)).length;
    if (arabicMatches >= 1) {
      score += 3;
      reasons.add('Arabic text');
    }
    ...
    if (RegExp(r'(CHIP|SMART|ELECTRONIC)', caseSensitive: false)
        .hasMatch(cleanText)) {
      score += 1;
    ...
    if (RegExp(r'U\.?A\.?E|EMIRATES|UNITED ARAB', caseSensitive: false)
        .hasMatch(cleanText)) {
      score += 2;
    ...
    if (RegExp(r'BACK|VERSO|REVERSE|خلف', caseSensitive: false)
        .hasMatch(cleanText)) {
      score += 1;
  ...
  bool _isTaxInvoiceOrBusinessDocument(String text) {
    ...
    int businessPatternCount =
        taxInvoicePatterns.where((pattern) => pattern.hasMatch(text)).length;
      ...
      return Scaffold(
        ...
        body: Center(
          child: Column(
            ...
            children: (_statusMessage.toLowerCase() ==
                ...
                ? [
                    ...
                    OutlinedButton(
                        ...
                        child: Text(
                          "Go to Settings",
                          style:
                              TS.f16.copyWith(color: AppConfigs.primaryColor),
                        ))
                  ]
                : [
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  border: Border.all(
                    color: _isAutoCapture
                        ? (_sideMatchesRequirement && _idCardDetected
                            ? Colors.green
                            : Colors.white)
                        : Colors.white,
                    width: 1.8,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      widget.captureType == 'front'
                          ? 'ALIGN ID FRONT (WITH PHOTO)'
                          : 'ALIGN ID BACK (WITHOUT PHOTO)',
                    ...
                    Text(
                      _isAutoCapture
                          ? 'Keep some distance for full capture'
              ...
              child: Column(
                children: [
                  Text(
                    _isAutoCapture
                        ? (!_sideMatchesRequirement
                            ? _getSideValidationMessage()
                            : _statusMessage)
                        : _statusMessage,
                    style: TextStyle(
                      color: _isAutoCapture && !_sideMatchesRequirement
                          ? Colors.red
                          : Colors.white,
                    ),
                  ),
