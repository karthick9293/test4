...
Future<void> transactionCancel({required String transactionRefNo, required String type}) async {
                        ...
                        return Column(
                          children: [
                            ...
                            Container(
                              ...
                              decoration:
                                  BoxDecoration(borderRadius: const BorderRadius.all(Radius.circular(5)), border: Border.all(color: AppConfigs.gd7)),
                              child: Text(
                            ...
                            textFormField(
                                ...
                                left: 10,
                                right: 10,
                                controller: TCancelControllers.remarksController,
                                hintText: 'Remarks'.tr,
                            ...
                            tc.transactionCancelStatus == false
                                ? roundButton(
                                    ...
                                    onTap: () async {
                                      ...
                                      //   accountController.update();
                                      // }else
                                      if (TCancelControllers.remarksController.text.trim().isEmpty) {
                                        TCancelControllers.remarksError = 'required'.tr;
                                        accountController.update();
                                      } else {
                                        tc
                                        ...
                                        TxnCancelFz? data = await ApiService().cancelTransaction(
                                            // referenceNo: TCancelControllers.referenceController.text,
                                            referenceNo: transactionRefNo,
                                            cancelRemarks: TCancelControllers.remarksController.text);

                                        if (data?.statusCode == "RMA-SUCCESS") {
                                          TCancelControllers.clear();
                                          if (Get.isDialogOpen == true) {
                                            Get.back();
                                            var headerMsg = data?.customMessageDetails.messageHeader;
                                            var descMsg = data?.customMessageDetails.messageDescription;
                                            showSuccessAlert(
                                              title: headerMsg,
                                              content: descMsg,
                                              function: () {
                                          ...
                                          if (Get.isDialogOpen == true) {
                                            Get.back();
                                            showFailAlert(content: data?.statusMessage ?? '');
                                          }
                                          tc
                                            ..transactionCancelStatus = false
                                            ..update();
