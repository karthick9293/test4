...
class ChangePasswordFormRulesPage extends StatefulWidget {
  ...
  @override
  State<ChangePasswordFormRulesPage> createState() => _ChangePasswordFormRulesPageState();
}

class _ChangePasswordFormRulesPageState extends State<ChangePasswordFormRulesPage> {
  ...
  void initializeControllers() {
    for (var field in mainController.resetPasswordFormFZ!.formFieldItem) {
      mainController.resetPasswordForm.textFieldControllers[field.FzParamName] = TextEditingController(text: '');
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<MainController>(builder: (mainController) {
      List<Widget> children() {
        List<Widget> l = [];
        List<FormFieldItem> fields = mainController.resetPasswordFormFZ!.formFieldItem.toList();
            ...
            case FieldType.username:
              l.add(
                textFormField(
                    ...
                    controller: mainController.resetPasswordForm.textFieldControllers[f.FzParamName],
                    ...
                    inputFormatters: f.Regex.isNotEmpty
                        ...
                        : null,
                    errorText: mainController.resetPasswordForm.errorTexts[f.FzParamName]!,
                    enabled: f.enabled,
                    onChanged: (v) async {
                      if (mainController.resetPasswordForm.errorTexts[f.FzParamName]!.isNotEmpty) {
                        mainController.resetPasswordForm.errorTexts[f.FzParamName] = '';
                        mainController.update();
                      }
                      if (f.maxLength != null) {
                        mainController.update();
                    ...
                    focusNode: mainController.resetPasswordForm.nodes[f.FzParamName],
                    onEditingComplete: () {
                      String? id = mainController.resetPasswordFormFZ?.formFieldItem
                          .toList()
                          .firstWhereOrNull((f0) => f0.index == f.index + 1 && (f0.FieldType == FieldType.text || f0.FieldType == FieldType.number))
                          ?.FzParamName;
                      Get.focusScope?.unfocus();
                      mainController.resetPasswordForm.nodes[id]?.requestFocus();
                    },
                    prefixText: f.Prefix),
              );
              break;
            case FieldType.number:
              ...
              l.add(
                textFormField(
                    ...
                    enabled: f.enabled,
                    obscure: mainController.resetPasswordForm.visibilities[f.FzParamName] == true,
                    obscuringCharacter: f.Masking != "" ? f.Masking : null,
                    trailing: f.Masking != ""
                        ? GestureDetector(
                            onTap: () async {
                              if (mainController.resetPasswordForm.visibilities[f.FzParamName]!) {
                                mainController
                                  ..resetPasswordForm.visibilities[f.FzParamName] = false
                                  ..update();
                                await Future.delayed(const Duration(seconds: 2));
                                mainController
                                  ..resetPasswordForm.visibilities[f.FzParamName] = true
                                  ..update();
                              }
                            },
                            child: Icon(
                              mainController.resetPasswordForm.visibilities[f.FzParamName] == true ? Icons.visibility_off : Icons.visibility,
                              size: 20,
                    ...
                    Padding(
                      ...
                      child: Text(
                          mainController.signUpAttributes.pwdStrength < 0.34
                              ? 'weak'.tr
                              : mainController.signUpAttributes.pwdStrength < 0.68
                                  ? 'Medium'.tr
                                  : 'Strong'.tr,
                          style: TS.f14),
                    ),
                      ...
                      child: Column(
                        children: [
                          textConstratint('contains_at_least_one_uppercase'.tr,
                              mainController.resetPasswordForm.textFieldControllers[f.FzParamName]!.text.contains(RegExp(r'[A-Z]'))),
                          textConstratint('contains_at_least_one_lowercase'.tr,
                              mainController.resetPasswordForm.textFieldControllers[f.FzParamName]!.text.contains(RegExp(r'[a-z]'))),
                          textConstratint('contains_at_least_one_number'.tr,
                              mainController.resetPasswordForm.textFieldControllers[f.FzParamName]!.text.contains(RegExp(r'[0-9]'))),
                          textConstratint('contains_at_least_one_special_char'.tr,
                              mainController.resetPasswordForm.textFieldControllers[f.FzParamName]!.text.contains(RegExp(r'[!@#\$&*~]'))),
                          textConstratint('Password length 8-$pwdMaxLength characters',
                              mainController.resetPasswordForm.textFieldControllers[f.FzParamName]!.text.length > 7)
                        ],
                      ),
                    ),
                  ],
                ...
                mainController.resetPasswordFormFZ == null
                    ...
                    : mainController.loading
                        ...
                        : roundButton(
                            ...
                            top: 20,
                            onTap: () async {
                              bool status = mainController.resetPasswordForm.validateNew(mainController.resetPasswordFormFZ!.formFieldItem);
                              ...
                              if (status) {
                                mainController
                                  ..loading = true
                                  ..update();
                                var formData = mainController.resetPasswordForm.buildJsonNew(mainController.resetPasswordFormFZ!.formFieldItem);
                                ...
                                String? s = await ApiService().changeMPINPassword(
                                  data: formData,
                                );
                                ...
                                Get.showSnackbar(GetSnackBar(
                                  messageText: Text(
                                    s == 'SUCCESS' ? '${'Password'.tr}${' changed Successfully'.tr}' : 'Invalid ${'Password'.tr}',
                                    style: TS.f14.copyWith(color: AppConfigs.w),
                                  ),
                                  backgroundColor: (s == 'SUCCESS' ? AppConfigs.dg : Colors.red).withOpacity(0.7),
